...
client.on('message', message => {                      
    if(!message.channel.guild) return;
       if(message.content.startsWith(prefix + 'Tf3el')) {
        let modlog = client.channels.find('name', 'Prove');
       if(!message.channel.guild) return message.channel.send('**هذا الأمر فقط للسيرفرات**').then(m => m.delete(5000));
       message.channel.sendMessage(`اضغط صح للتفعيل`).then(msg => {
       ...
       let active = msg.createReactionCollector(activeFilter, { time: 15000 });
                               ...
                               active.on("collect", r => {
                                   ...
                                   message.member.removeRole(message.guild.roles.find("name", "no Member"));
                                   msg.delete();
